...
-- generate encryption and decryption code that console the output 
-- Users table
...
-- Business profiles (business-specific info)
...
CREATE TABLE tbl_search_history (
    ...
    search_type VARCHAR(20) CHECK (search_type IN ('offer', 'business', 'category')) DEFAULT 'offer', --offer(home and offers page), business(on map), category(category page)
