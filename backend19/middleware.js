...
const checkToken = async function (req, res, next) {
    try {
        ...
        if (!token) {
            return sendResponse(req, res, 401, '-1', { keyword: 'token_required', components: {} }, {});
        }

        // Try both possible secret keys for backward compatibility
        let decoded;
        try {
            decoded = jwt.verify(token, process.env.JWT_SECRET_KEY || process.env.JWT_SECRET || "your_jwt_secret");
        } catch (err) {
        ...
        // For normal user, check user existence
