...
const offer_model = {
    ...
    try {
      ...
      if (duration > membership.visibility_days) {
        return sendResponse(
          ...
          { keyword: "duration_exceeds_limit" },
          {
      ...
      if (membership.offer_quantity_limit && quantity_available > membership.offer_quantity_limit) {
        return sendResponse(
          ...
          200,
          responseCode.OPERATION_FAILED,
          { keyword: "quantity_exceeds_limit" },
          {
  ...
  async listOffers(req, res) {
    try {
      ...
      let user_id = type == "user" ? req.body.user_id : req.user.id
      ...
      // Get current day (0-6 where 0 is Sunday) and current time in UTC
      const currentDate = new Date()
      const currentDay = currentDate.getUTCDay() // 0 (Sunday) to 6 (Saturday)
      const currentTime = currentDate.toISOString().substring(11, 19) // HH:MM:SS format
      ...
      // PRIORITY ORDERING: R+ offers first, then regular offers listed in R+ store
      let orderBy = `
        CASE 
          WHEN o.is_rplus_offer = TRUE THEN 1
          WHEN o.is_listed_in_rplus = TRUE THEN 2
          ELSE 3
        END ASC,
      `
      ...
      // Additional sorting after priority
      switch (sort_by) {
      ...
      const mainQuery = `
        SELECT 
          ...
          o.start_date, o.end_date, o.created_at, o.is_listed_in_rplus, o.is_rplus_offer,
      ...
      const offersResult = await pool.query(mainQuery, queryParams)
  ...
  async getOfferDetails(req, res) {
    try {
      ...
      const recommendedResult = await pool.query(recommendedQuery, [offer.business_subcategory_id, user_id, offer_id])
    ...
    try {
      ...
      if (currency && currency !== currentOffer.currency && !membership.can_update_currency) {
        return sendResponse(
          ...
          403,
          responseCode.OPERATION_FAILED,
          { keyword: "currency_update_not_allowed" },
          {},
        )
      }
