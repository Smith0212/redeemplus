    ...
    async login(req, res) {
        try {
            ...
            const checkDeviceQuery = `
                ...
                WHERE device_token = $1 AND user_token IS NOT NULL
            `;
            const deviceResult = await pool.query(checkDeviceQuery, [device_token]);
            if (deviceResult.rows.length > 0 && deviceResult.rows[0].user_id !== user.user_id) {
            ...
            if (!user.is_verified) {
                ...
                return sendResponse(req, res, 200, responseCode.OTP_NOT_VERYFIED, { keyword: 'not_verified_otp_sent' }, safeUser);
            }
            ...
            if (user.step < 3) {
                return sendResponse(req, res, 200, responseCode.CODE_NULL, { keyword: 'choose_subscription_plan' }, user);
            }
